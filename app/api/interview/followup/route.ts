...
async function resolveModelName(apiKey: string) {
  const envModel = process.env.GEMINI_FOLLOWUP_MODEL ?? process.env.GEMINI_MODEL;
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Failed to list models (${res.status}). Set GEMINI_FOLLOWUP_MODEL explicitly. ${text}`
    );
  }
  ...
  if (!picked) {
    throw new Error(
      "No generateContent-capable models found. Call /api/interview/models or set GEMINI_FOLLOWUP_MODEL."
    );
  }
...
async function resolveModelCandidates(apiKey: string) {
  const preferred = process.env.GEMINI_FOLLOWUP_MODEL ?? process.env.GEMINI_MODEL;
  const now = Date.now();
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const system =
      "You are Pixie, an AI interviewer. Ask ONE concise follow-up question based on the candidate's answer. " +
      "Do not use markdown. Do not add extra commentary. Do not ask multiple questions.";

    const prompt =
      `Base question: ${question}\n` +
      `Candidate answer: ${answer}\n\n` +
      "Ask exactly ONE follow-up question that digs deeper into something they said.";
    ...
    for (const model of models) {
      ...
      if (!res.ok) {
        ...
        lastError = json.error?.message ?? "Gemini follow-up failed.";
      ...
      if (!text) {
        return NextResponse.json(
          { error: "Gemini returned an empty follow-up." },
          { status: 500 }
        );
      }
      ...
      return NextResponse.json({ followup: text, model });
    }
    ...
    return NextResponse.json(
      {
        error:
          lastError ??
          (finalStatus === 429
            ? "Quota exceeded for follow-up."
            : "No supported model accepted the follow-up request."),
        status: lastStatus,
      },
      { status: finalStatus }
    );
  } catch (e) {
    const message = e instanceof Error ? e.message : "Gemini follow-up failed.";
